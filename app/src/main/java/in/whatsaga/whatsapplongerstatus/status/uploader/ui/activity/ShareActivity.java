...
import static androidx.multidex.BuildConfig.APPLICATION_ID;
...
public class ShareActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    public static void start(Context context, String which) {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        extraText = "Read Deleted Messages,Recover Media file : \n " + appLink;
    ...
    public void shareImage(String packge, String path) {
        Uri uri;
        if (type.equals(Constants.SAVED)) {
            if (Common.isRorAbove()) {
                uri = FileProvider.getUriForFile(ShareActivity.this,
                        APPLICATION_ID + ".provider", new File(path));
            } else {
                uri = Uri.parse(path);
            }
        } else {
    ...
    public void shareVideo(String packge, String path) {
        ...
        if (type.equals(Constants.SAVED)) {
            if (Common.isRorAbove()) {
                uri = FileProvider.getUriForFile(ShareActivity.this,
                        APPLICATION_ID + ".provider", new File(path)); //issue
            } else {
                uri = Uri.parse(path);
            }
        } else {
