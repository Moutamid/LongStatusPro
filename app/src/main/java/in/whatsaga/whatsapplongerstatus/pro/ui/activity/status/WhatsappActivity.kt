...
import `in`.whatsaga.whatsapplongerstatus.pro.adapters.StatusAdapter
...
class WhatsappActivity : AppCompatActivity(), StatusAdapter.OnItemClick, CompoundButton.OnCheckedChangeListener,
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (Build.VERSION.SDK_INT < 30) {
            path = Common.whatsAppFolderStatus
            if (!path!!.exists()) {
                path = Common.whatsAppFolderStatus11
            }
        } else {
            //android 11
            path = Common.whatsAppFolderStatus11
    ...
    private fun getList() {
        ...
        adapter = StatusAdapter(this, list, this, Constants.WHATSAPP)
    ...
    fun delete(launcher: ActivityResultLauncher<IntentSenderRequest>?, uri: Uri) {
        val contentResolver = this@WhatsappActivity.contentResolver
        try {
